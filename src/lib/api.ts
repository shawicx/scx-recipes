...
export const saveHealthProfile = async (profile: HealthProfile): Promise<string> => {
...
export const getRecommendationById = async (id: string): Promise<RecommendationItem | null> => {
...
export const getDietHistory = async (params: GetHistoryParams): Promise<DietEntry[]> => {
  return await invoke('get_diet_history', { params });
};

export const updateDietEntry = async (params: UpdateDietEntryParams): Promise<boolean> => {
...
export const searchRecipes = async (params: SearchRecipesParams): Promise<Recipe[]> => {
